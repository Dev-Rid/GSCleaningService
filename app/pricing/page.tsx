      ...
      <div className="relative pt-24">
        <div className="max-w-7xl mx-auto px-6 pb-20">
          ...
          <section className={`pb-16 transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="grid md:grid-cols-3 gap-8">
              {pricingCategories.map((cat, i) => (
                <div
                  key={i}
                  className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-cyan-100 hover:border-cyan-300 hover:-translate-y-2 overflow-hidden"
                >
                  {/* Card Header */}
                  ...
                  <div className="p-6">
                    {/* Column Headers */}
                    <div className="flex justify-between items-center pb-3 mb-3 border-b border-cyan-100">
                    ...
                    <div className="space-y-3">
                      {cat.rows.map((row, j) => (
                        <div
                          key={j}
                          className="flex justify-between items-center bg-cyan-50 hover:bg-cyan-100 rounded-xl px-4 py-3 border border-cyan-100 transition-colors duration-200"
                        >
                          <span className="text-slate-700 text-sm font-medium">{row.label}</span>
                          <span className="text-cyan-700 font-bold text-sm whitespace-nowrap ml-4">{row.price}</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <section className={`transition-all duration-1000 delay-600 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="bg-gradient-to-br from-cyan-600 to-blue-700 rounded-3xl p-10 md:p-16 shadow-2xl relative overflow-hidden text-center">
              <div className="absolute inset-0 opacity-10">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <a
                    href="/contact"
                    className="px-8 py-4 bg-white text-cyan-700 rounded-xl font-semibold text-lg hover:bg-cyan-50 hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
                  >
                    Get a Free Quote
                  </a>
                  <a
